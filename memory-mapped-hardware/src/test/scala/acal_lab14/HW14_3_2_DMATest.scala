...
class topTest extends AnyFlatSpec
    ...
    with AXITester{
        val idWidth = HW14_3_2_DMA_Config.s_id_width
        val addrWidth = HW14_3_2_DMA_Config.addr_width
        val dataWidth = HW14_3_2_DMA_Config.data_width
    "HW14_3_2_DMA CPU" should "program DMA and SA to compute matrix multiplication" in {
        test(new top()).withAnnotations(Seq(
            ...
            val lines    = Source.fromFile(HW14_3_2_DMA_Config.instr_asm_path).getLines.toList
            ...
            for(i <- 0 until 12) {
                ...
                dut.io.tb_slave.ar.enqueue(genAXIAddr(BigInt(HW14_3_2_DMA_Config.Mem_Base_ADDR + 4 * i)))
                val addr = (HW14_3_2_DMA_Config.Mem_Base_ADDR + 4 * i).toHexString
